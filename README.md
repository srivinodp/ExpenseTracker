...
- 🥧 **Category Breakdown (Pie Chart)**: Beautiful Canvas-based expense distribution chart featuring a dynamic, compact header dropdown. It automatically scans all logged transactions to list every custom month with logged expenses (plus an **Overall** statistics option), re-rendering chart slices and legend bullets in real-time.
